...
# Copyright: (c) 2020, Harry Saryan  <hs-hub-world@github> (Updating script to accomdate Access Denied for Wait-Process cmdlet )
...
if ($state -eq "present" ) {
    ...
    if ($Processes.count -gt 0 ) {
        try {
            ...
            #Harry Version since Wait-Process would through access denied
            #Get-Process  -id "350483" -ErrorAction SilentlyContinue
            while (Get-Process -id "$($Processes.pid)" -ErrorAction SilentlyContinue)
            {
                sleep -seconds 2;
                if (((Get-Date) - $module_start).TotalSeconds -gt $timeout) {
                    $module.Result.elapsed = ((Get-Date) - $module_start).TotalSeconds
                    $module.FailJson("Timed out while waiting for process(es) to stop")
                }
            }
