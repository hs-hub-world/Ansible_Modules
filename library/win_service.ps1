...
# New Version: (modified/fix to accomdate passwords) original ver was using wmisvc changed it to sc.exe
...
Function Set-ServiceAccount($wmi_svc, $username_sid, $username, $password) {
    ...
    if ($actual_sid -ne $username_sid) {
        ...
        if (-not $check_mode) {
            #TCW Version - update pwd using sc first before getting service status
            sc.exe config "$($wmi_svc.Name)" type= own obj= "$username" password= "$password" |Out-Null
